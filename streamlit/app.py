...
[data-testid="stAudio"]{margin-top:4px;}
...
(function drawWorld(){
    ...
    var BLOCKS={
        grass:['#5D9E2F','#63A832','#579429'],dirt:['#866043','#7A5538','#8C6850'],
        stone:['#5A5A5A','#626262','#545454'],coal:['#3A3A3A','#444444','#333333'],
        iron:['#8C7E6A','#96886F','#857561'],gold:['#C8A42A','#D4B030','#BC9824'],
        diamond:['#2AA8B8','#30B4C0','#26A0B0'],wood:['#7A5518','#6B4A14','#83601C'],
        leaves:['#3D6E1A','#447820','#4A8224'],water:['#1A4A8C','#1E5098','#184480'],
    ...
    function drawTree(tx,ty){
        for(var i=0;i<4;i++){cx.fillStyle=rnd(BLOCKS.wood);cx.fillRect(tx+B*0.375,ty-i*B,B*0.25,B);}
        var lp=[[-1,-4],[0,-4],[1,-4],[-2,-3],[-1,-3],[0,-3],[1,-3],[2,-3],
                [-2,-2],[-1,-2],[0,-2],[1,-2],[2,-2],[-1,-1],[0,-1],[1,-1]];
        lp.forEach(function(p){cx.fillStyle=rnd(BLOCKS.leaves);cx.fillRect(tx+p[0]*B*0.5,ty+p[1]*B*0.5,B*0.5,B*0.5);});
    }
    function drawCreeper(x,baseY){
        var p=6;
        cx.fillStyle='#5DBF5D';cx.fillRect(x-p*3,baseY-p*14,p*6,p*6);
        cx.fillStyle='#000';cx.fillRect(x-p*2,baseY-p*13,p,p);cx.fillRect(x+p,baseY-p*13,p,p);
        cx.fillRect(x-p*2,baseY-p*10,p,p);cx.fillRect(x+p,baseY-p*10,p,p);cx.fillRect(x-p,baseY-p*9,p*2,p);
        cx.fillStyle='#4CAF50';cx.fillRect(x-p*3,baseY-p*8,p*6,p*8);
        cx.fillStyle='#3D8C3D';cx.fillRect(x-p*3,baseY,p*2,p*3);cx.fillRect(x+p,baseY,p*2,p*3);
    }
    ...
    function render(){
        ...
        for(var s=0;s<220;s++){
            ...
            cx.fillRect(Math.random()*W,Math.random()*H*0.55,Math.random()<0.3?2:1,1);
        }
        ...
        for(var c=0;c<cols;c++){
            ...
            for(var r=top;r<rows;r++){
                ...
                else{var rv=Math.random();type=rv<0.07?'coal':rv<0.04?'iron':rv<0.015?'gold':rv<0.004?'diamond':'stone';}
                drawBlock(x2,y2,type);
            }
            cx.fillStyle='#79C240';cx.fillRect((c-1)*B,top*B,B,5);
            if(c>2&&c<cols-2&&c%7===3&&Math.random()<0.75)drawTree((c-1)*B,top*B);
        }
        for(var w=0;w<4;w++){
            var wc=Math.floor(cols*0.1+w*cols*0.25),wy=gy[Math.min(wc,cols-1)]+1;
            for(var wd=0;wd<3;wd++){cx.fillStyle=rnd(BLOCKS.water);cx.fillRect((wc+wd-1)*B,wy*B,B,B);}
        }
        for(var t=0;t<6;t++){
            var tc=Math.floor(cols*0.08+t*(cols/6));
            drawTorch((tc-1)*B+B/2,(gy[Math.min(tc,cols-1)]-1)*B);
        }
        drawCreeper(W*0.18,gy[Math.floor(W*0.18/B)]*B);
        drawCreeper(W*0.6,gy[Math.floor(W*0.6/B)]*B);
        drawCreeper(W*0.87,gy[Math.floor(W*0.87/B)]*B);
        cx.fillStyle='rgba(5,10,20,0.38)';cx.fillRect(0,0,W,H);
    }
    render();
